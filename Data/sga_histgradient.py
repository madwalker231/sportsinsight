...
# Tiny, predeclared grid (controlled; not “besting”)
PARAM_GRID = {
...
def fetch_gamelog(player_id: int, season: str) -> pd.DataFrame:
    ...
    keep = ["GAME_ID","GAME_DATE","MATCHUP","TEAM_ID","MIN","PTS","REB","AST","FGA","FG3A","FTA"]
    keep = [c for c in keep if c in df.columns]
    if not keep: return pd.DataFrame()
    df = df[keep].copy()
    if "GAME_DATE" not in df.columns or df.empty: return pd.DataFrame()
...
def parse_opp_abbr(matchup: str):
    # "OKC vs LAL" or "OKC @ LAL" -> ("LAL", is_home 1/0)
...
def build_features(df_all: pd.DataFrame) -> pd.DataFrame:
    ...
    # Opponent DEF_RATING per season (fallback to league-median)
    def_maps = {}
    ...
    # Rolling L5 (shifted to avoid lookahead)
...
def save_plots(pred_df: pd.DataFrame):
    ...
    plt.plot(dates, pred_df["PRED_PTS"], marker="o", label="HistGB")
    plt.plot(dates, pred_df["L5_BASELINE"], linestyle="--", label="L5 baseline")
    plt.xticks(rotation=45, ha="right")
    plt.title("Shai Gilgeous-Alexander – Predicted vs Actual (2024–25)")
    plt.ylabel("Points")
    plt.tight_layout()
    plt.legend()
...
if __name__ == "__main__":
    ...
    print("\n=== CV (2013–24) – HistGradientBoosting ===")
    ...
    out.to_csv("sga_hgb_2024_25_predictions.csv", index=False)
    print("Saved CSV: sga_hgb_2024_25_predictions.csv")
