...
# Tiny, predeclared grid (no optimization theater)
PARAM_GRID = {
...
def parse_opp_abbr(matchup: str):
    # "BOS vs LAL" or "BOS @ LAL" -> ("LAL", is_home 1/0)
...
def save_plots(pred_df: pd.DataFrame):
    ...
    plt.plot(dates, pred_df["PRED_PTS"], marker="o", label="KNN")
    plt.plot(dates, pred_df["L5_BASELINE"], linestyle="--", label="L5 baseline")
    plt.xticks(rotation=45, ha="right")
    plt.title("Jayson Tatum – Predicted vs Actual (2024–25)")
    plt.ylabel("Points")
    plt.tight_layout()
    plt.legend()
...
if __name__ == "__main__":
    ...
    out.to_csv("tatum_knn_2024_25_predictions.csv", index=False)
    print("Saved CSV: tatum_knn_2024_25_predictions.csv")
